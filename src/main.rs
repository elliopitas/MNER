...
async fn main() -> Result<()> {
	...
	match args.command {
		Commands::Run { config, output, ssh_keys } => {
			// SAFETY: We are setting environment variables early in the program execution.
			// This is required for the ssh-agent to function correctly with the child processes.
			let _agent = unsafe { setup_ssh_agent(&ssh_keys).await? };
